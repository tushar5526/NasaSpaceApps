...
namespace UnityARInterface
{
	public class ARFocusSquare : MonoBehaviour {
        ...
        public GameObject[] exploreCubes;
		...
		void Update () {
			...
			if (Physics.Raycast(ray, out hit, maxRayDistance, collisionLayerMask)) {
                ...
                if (hit.transform.tag == "ExploreCube")
                {
                    ...
                    {
                        Debug.Log("last Gazed " + _lastGazedUpon.name + " current hit " + hit.transform.name);
                    }
                    if (_lastGazedUpon != null && _lastGazedUpon.name != hit.transform.name)
                    {

                        //hit.transform.GetComponenta<>().isGazing = true;
                        //hit.transform.GetComponent<Exp>  
                        _lastGazedUpon.transform.gameObject.GetComponent<ExploreCubesController>().isGazing = false;
                    }
                    _lastGazedUpon = hit.transform;
                    hit.transform.gameObject.GetComponent<ExploreCubesController>().isGazing = true;
                }


                return;
			}
            gaze.transform.localScale = Vector3.Lerp(gaze.transform.localScale, new Vector3(0.5f, 0.5f, 0.5f), 0.2f);
